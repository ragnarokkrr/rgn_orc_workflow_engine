...
public enum WorkflowStateAssertionAssertions implements WorkflowAssertions.StateAssertion {
    ...
    WorkflowStateAssertionAssertions(WorkflowAssertions.StateAssertion delegate) {
    ...
    private static class Strategy {
        static final WorkflowAssertions.StateAssertion WORKFLOW_ALREADY_CONFIGURED =
                (workflowRoot, action) -> Assert.state(Objects.isNull(workflowRoot.getConfiguration()), String.format(
                        "Workflow already configured (%s, %s)", workflowRoot.getId(), workflowRoot.getCustomerRequest()));

        static final WorkflowAssertions.StateAssertion EXECUTION_PLAN_REQUIREMENTS =
                (workflowRoot, action) -> Assert.state(Objects.nonNull(workflowRoot.getConfiguration())
                                && WorkflowStatus.INSTANTIATED == workflowRoot.getStatus(),
                        String.format("Execution Plan requirements invalid (configuration=%s, status=%s). Request: %s",
                                !Objects.isNull(workflowRoot.getConfiguration()), workflowRoot.getStatus(), workflowRoot.getCustomerRequest()));

        static final WorkflowAssertions.StateAssertion CONFIGURATION_REQUIREMENTS =
                (workflowRoot, action) ->
                        Assert.state(Objects.nonNull(workflowRoot.getConfiguration())
