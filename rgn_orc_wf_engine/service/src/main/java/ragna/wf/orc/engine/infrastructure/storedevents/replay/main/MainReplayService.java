...
public class MainReplayService {
  ...
  Mono<MainReplayContextVo> findHandler(final MainReplayContextVo mainReplayContextVo) {
    ...
    final var mainStoredEventReplayerBean =
        Try.ofCallable(() -> Optional.of(applicationContext.getBean(matchedReplayer.get())))
  ...
  Mono<MainReplayContextVo> evaluateTaskActivationCriteria(
      final MainReplayContextVo mainReplayContextVo) {
    if (mainReplayContextVo.getMainStoredEventReplayerCallback().isEmpty()
        || mainReplayContextVo.getReplayResult().getReplayResultType()
            == MainReplayContextVo.ReplayResultEnum.NO_HANDLER_FOUND) {
      LOGGER
          .warn()
          .log(
              "No MainStoredEventReplayerCallback found '{}', skipping {}",
              mainReplayContextVo.getReplayResult(),
              mainReplayContextVo.getStoredEventVo());
      return Mono.just(mainReplayContextVo);
    }

    final var mainStoredEventReplayerCallback =
        (MainStoredEventReplayerCallback<? extends DomainEvent>)
  ...
  Mono<MainReplayContextVo> replay(final MainReplayContextVo mainReplayContextVo) {
    ...
    return mainStoredEventReplayerCallback
        .doReplay(mainReplayContextVo)
        .map(MainReplayContextVo::processed);
  }
  ...
  Mono<MainReplayContextVo> publishEventIfNecessary(final MainReplayContextVo mainReplayContextVo) {
    if (mainReplayContextVo.getCriteriaEvaluationResult().isEmpty()) {
      LOGGER
          .warn()
          .log(
              "Activation criteria criteria is empty for {}: ",
              mainReplayContextVo.getStoredEventVo());
      return Mono.just(mainReplayContextVo);
    }

    final var criteriaEvaluationResult = mainReplayContextVo.getCriteriaEvaluationResult().get();
    if (criteriaEvaluationResult.getCriteriaResultType()
        != CriteriaEvaluationResult.CriteriaResultType.MATCHED) {
      LOGGER
          .warn()
          .log(
              "Activation criteria unmatched for {}: ",
              mainReplayContextVo.getStoredEventVo(),
    ...
    final var mainStoredEventReplayerCallback =
        (MainStoredEventReplayerCallback<? extends DomainEvent>)
            mainReplayContextVo.getMainStoredEventReplayerCallback().get();
    return mainStoredEventReplayerCallback
        .publish(mainReplayContextVo)
        .map(MainReplayContextVo::published)
        .doOnNext(
            mainReplayContextVo1 -> LOGGER.info().log("PUBLISHED!!!! {}", mainReplayContextVo1));
  }
  ...
  Mono<MainReplayContextVo> markStoredEventProcessingStatus(
    ...
    if (updateStoredEventCommand.isEmpty()) {
      LOGGER
          .debug()
          .log(
              "Stored Event Processing result not mapped for event store update: {}",
              mainReplayContextVo.getStoredEventVo());
      return Mono.just(mainReplayContextVo);
    }
  ...
  Mono<MainReplayContextVo> dispatchToSideReplay(MainReplayContextVo mainReplayContextVo) {
    final var secondaryReplayContextVo =
        SecondaryReplayContextVo.createContext(mainReplayContextVo.getStoredEventVo());
    sideReplayContextVoReplayProcessor.onNext(secondaryReplayContextVo);
    return Mono.just(mainReplayContextVo);
  }

  Optional<UpdateStoredEventCommand> mapReplayProcessingResult(
      final MainReplayContextVo mainReplayContextVo) {
    final var replayResultType = mainReplayContextVo.getReplayResult().getReplayResultType();
    LOGGER
        .debug()
        .log(
            "mapReplayProcessingResult() replayResultType={}, {}",
            replayResultType,
            mainReplayContextVo.getStoredEventVo());
    return switch (replayResultType) {
      case PROCESSED, NO_HANDLER_FOUND -> Optional.of(
          buildUpdateStoredEventCommand(
              mainReplayContextVo, UpdateStoredEventCommand.TargetState.PROCESSED));
      case PUBLISHED -> Optional.of(
          buildUpdateStoredEventCommand(
              mainReplayContextVo, UpdateStoredEventCommand.TargetState.PUBLISHED));
      case UNMATCHED -> Optional.of(
          buildUpdateStoredEventCommand(
              mainReplayContextVo, UpdateStoredEventCommand.TargetState.UNPUBLISHED));
      case ERROR -> Optional.of(
          buildUpdateStoredEventCommand(
              mainReplayContextVo, UpdateStoredEventCommand.TargetState.FAILED));
      case PROCESSING, IGNORED, MATCHED -> Optional.empty();
    };
  }

  private UpdateStoredEventCommand buildUpdateStoredEventCommand(
      final MainReplayContextVo mainReplayContextVo,
      final UpdateStoredEventCommand.TargetState failed) {
