...
public class TaskActivationCriteriaService {
  ...
  public Mono<CriteriaEvaluationResult> matchTaskCriteria(
    ...
    final var criterionResultCallMonoList =
        criteriaEvaluationQuery.getCriteriaList().stream()
            .map(
                criterion ->
            ...
            .map(
                criterionMonoFunction ->
                    criterionMonoFunction.apply(criteriaEvaluationQuery.getCustomerId()))
            .collect(Collectors.toList());

    final var criterionResultArrayMono =
        Mono.zip(
            criterionResultCallMonoList,
            objects ->
                Arrays.stream(objects)
                    .map(object -> (CriteriaEvaluationResult.CriterionResult) object)
                    .collect(Collectors.toList()));
    return criterionResultArrayMono.map(
        criteriaEvaluationResults ->
            CriteriaEvaluationResult.builder()
                .customerId(criteriaEvaluationQuery.getCustomerId())
                .addAllCriteriaResult(criteriaEvaluationResults)
                .build());
  }

  final Function<String, Mono<CriteriaEvaluationResult.CriterionResult>>
      callCriterion1ServiceFunctionMono(final CriteriaEvaluationQuery.Criterion criterion) {
    return (customerId) ->
        taskActivationCriterion1Service
            .findByCustomerId(Criterion1Query.builder().customerId(customerId).build())
  ...
  final Function<String, Mono<CriteriaEvaluationResult.CriterionResult>>
      callCriterion2ServiceFunctionMono(final CriteriaEvaluationQuery.Criterion criterion) {
    return (customerId) ->
        taskActivationCriterion2Service
            .findByCustomerId(Criterion2Query.builder().customerId(customerId).build())
            .map(Criterion2ResponseVo::getValue)
            .map(value -> this.evaluateCriterion(value, criterion))
            .onErrorResume(throwable -> Mono.just(newCriterionResultError(criterion)));
  }

  private CriteriaEvaluationResult.CriterionResult evaluateCriterion(
      final Long value, final CriteriaEvaluationQuery.Criterion criterion) {
    final var matched =
        switch (criterion.getOrder()) {
  ...
  final Function<String, Mono<CriteriaEvaluationResult.CriterionResult>> invalidCriterion(
      final CriteriaEvaluationQuery.Criterion criterion) {
    return (customerId) ->
        Mono.just(
            CriteriaEvaluationResult.CriterionResult.builder()
                ...
                .resultType(
                    CriteriaEvaluationResult.CriterionResult.CriterionResultType.INVALID_CRITERION)
                .build());
  }

  private CriteriaEvaluationResult.CriterionResult newCriterionResultError(
      CriteriaEvaluationQuery.Criterion criterion) {
    return CriteriaEvaluationResult.CriterionResult.builder()
        .id(criterion.getId())
        .name(criterion.getName())
        .value(StringUtils.EMPTY)
